...
export default function DashboardPage() {
  ...
  if (!stats) {
    return <div className="flex justify-center items-center min-h-screen">No stats available.</div>;
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12 md:py-20">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-12 md:mb-16 gap-6">
          ...
          <div className="flex items-center gap-4">
            <button disabled className="glass-card px-4 py-2 rounded-full text-sm text-warm-300 flex items-center gap-2 opacity-50 cursor-not-allowed">
            ...
            <button disabled className="btn-premium bg-gradient-to-r from-accent-blue to-accent-purple text-white px-6 py-3 rounded-xl font-semibold opacity-50 cursor-not-allowed">
              새 프로젝트 추가
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          ...
          <div className="glass-card p-6 text-center shadow-premium-sm">
            <div className="gradient-text text-4xl font-bold mb-1">{formatViews(stats.totalLikes)}</div>
            <div className="text-warm-500 text-sm uppercase tracking-wider">총 좋아요</div>
          </div>
           <div className="glass-card p-6 text-center shadow-premium-sm">
            <div className="gradient-text text-4xl font-bold mb-1">{stats.newFollowers}</div>
            <div className="text-warm-500 text-sm uppercase tracking-wider">새 팔로워</div>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {topProjects?.map((project: any) => (
                  <div key={project.id} className="glass-card p-4 flex items-center justify-between hover:shadow-glow-blue transition-shadow">
                    <div>
                      <h3 className="font-semibold text-white">{project.title}</h3>
                      <p className="text-sm text-warm-400">{new Date(project.date).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold">{formatViews(project.views)} views</div>
                      <Link to={`/project/${project.id}`} className="text-sm text-accent-blue hover:underline">View</Link>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {notifications?.map((notification: any) => (
                <div key={notification.id} className="glass-card p-3 text-sm">
                  <p className="text-warm-300">{notification.message}</p>
                  <p className="text-xs text-warm-500 text-right">{new Date(notification.date).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
          </div>
